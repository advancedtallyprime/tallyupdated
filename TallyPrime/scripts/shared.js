...
fetch("/TallyPrime/assets/header.html")
    .then(res => res.text())
    .then(data => {
...
function renderSidebar() {
    ...
    advancedTallyCourse.forEach(module => {
        ...
        module.topics.forEach(topic => {
            ...
            html += `
                <li>
                    <a href="/TallyPrime/basic-tally/${module.folder}/${topic}.html">
                        ${title}
                    </a>
                </li>
            `;
...
function getCurrentTopicData() {
    ...
    for (const module of advancedTallyCourse) {
        for (let i = 0; i < module.topics.length; i++) {
            ...
            const url = `/TallyPrime/basic-tally/${module.folder}/${topic}.html`;
...
function renderBreadcrumbs() {
    ...
    container.innerHTML = `
        ...
        <span>›</span>
        <a href="/Pages/basic-tally.html">Basic Tally</a>
        <span>›</span>
...
function renderModuleProgress() {
    ...
    current.module.topics.forEach((topic, index) => {
        ...
        html += `
            <li class="${active}">
                <a href="/TallyPrime/basic-tally/${current.module.folder}/${topic}.html">
                    ${toTitleCase(topic)}
                </a>
            </li>
        `;
...
function renderTopicProgress() {
    ...
    html += `
        <a class="step-control" href="/TallyPrime/basic-tally/${current.module.folder}/${topics[0]}.html" title="First Topic">«</a>
    `;

    if (current.index > 0) {
        html += `
            <a class="step-control" href="/TallyPrime/basic-tally/${current.module.folder}/${topics[current.index - 1]}.html" title="Previous Topic">‹</a>
        `;
    }
    ...
    topics.forEach((topic, index) => {
        ...
        html += `
            <a class="topic-step ${active}" 
               ...
               href="/TallyPrime/basic-tally/${current.module.folder}/${topic}.html">
                ${index + 1}
            </a>
        `;
    });
    ...
    if (current.index < topics.length - 1) {
        html += `
            <a class="step-control" href="/TallyPrime/basic-tally/${current.module.folder}/${topics[current.index + 1]}.html" title="Next Topic">›</a>
        `;
    }

    html += `
        <a class="step-control" href="/TallyPrime/basic-tally/${current.module.folder}/${topics[topics.length - 1]}.html" title="Last Topic">»</a>
    `;
...
function renderPrevNext() {
    ...
    if (current.index > 0) {
        ...
        html += `
            <a class="prev-topic" href="/TallyPrime/basic-tally/${current.module.folder}/${prev}.html">
                ← ${toTitleCase(prev)}
            </a>
        `;
    }
    ...
    if (current.index < current.module.topics.length - 1) {
        ...
        html += `
            <a class="next-topic" href="/TallyPrime/basic-tally/${current.module.folder}/${next}.html">
                ${toTitleCase(next)} →
            </a>
        `;
    }
...
function initMobileNavigation() {
    ...
    // Ensure overlay div exists for backdrop closing
